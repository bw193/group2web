...
export default function ProductCard({
  ...
  return (
    <div className="group card-hover">
      {/* Image */}
      <Link href={`/${locale}/products/${slug}`} className="block relative aspect-[4/5] overflow-hidden bg-sand mb-5">
        <Image
          src={getUploadUrl(imageUrl)}
          alt={name}
          fill
          sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw"
          className="object-cover transition-transform duration-[1s] ease-out-expo group-hover:scale-105"
          loading="lazy"
        />
        <div className="product-image-overlay" />
        {isFeatured && (
          <span className="absolute top-4 left-4 bg-bronze text-cream text-[10px] font-body font-medium px-3 py-1.5 uppercase tracking-[0.15em]">
            {t('featured')}
          </span>
        )}
        <div className="absolute bottom-4 right-4 w-10 h-10 flex items-center justify-center bg-cream/90 text-ink opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-out-expo">
