...
const ABOUT_HE_CONTENT = `<p>שנוסדה בשנת 2005, חברת Jiaxing Chengtai Mirror Co., Ltd. היא יצרנית מובילה המתמחה בפתרונות מראות יוקרתיים, כולל מראות LED, מראות אמבטיה ומראות גוף מלא. המתקן המתקדם שבבעלותנו משתרע על פני 48,000 מ״ר בג׳יאשינג, ז׳ג׳יאנג — ממוקם אסטרטגית במרחק 60 ק״מ בלבד מנמל שנגחאי ללוגיסטיקה גלובלית יעילה.</p>
...
async function main() {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.insert(aboutPage).values({
      ...
      factorySize: enAbout?.factorySize ?? '48,000㎡',
